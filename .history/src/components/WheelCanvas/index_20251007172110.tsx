...
const drawWheel = (
  ...
  if (canvas?.getContext('2d')) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const segmentStartAngle = startAngle; // Save for decorative images
      ...
      // DECORATIVE IMAGES (e.g., stars) - Draw after content so they appear on top
      if (style?.decorativeImages && style.decorativeImages.length > 0) {
        const currentStartAngle = segmentStartAngle;
        const currentArc = arc;

        style.decorativeImages.forEach((decorativeImg, idx) => {
          // Use segment index for consistent positioning
          const seed = i * 1000 + idx * 10;
          const pseudoRandom1 = ((seed * 9301 + 49297) % 233280) / 233280;
          const pseudoRandom2 = ((seed * 9307 + 49313) % 233287) / 233287;

          // Position decorative images within the segment
          const decorRadius =
            insideRadius +
            (outsideRadius - insideRadius) * (0.25 + pseudoRandom1 * 0.5);
          const decorAngle =
            currentStartAngle + currentArc * (0.15 + pseudoRandom2 * 0.7);

          const x = centerX + decorRadius * Math.cos(decorAngle);
          const y = centerY + decorRadius * Math.sin(decorAngle);

          const size = (decorativeImg.size || 20) * 2; // Scale for canvas
          const opacity = decorativeImg.opacity ?? 0.8;

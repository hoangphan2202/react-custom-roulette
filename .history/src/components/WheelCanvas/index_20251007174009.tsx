...
const drawWheel = (
  ...
  if (canvas?.getContext('2d')) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (backgroundImage) {
        // Save current state and clip to segment
        ctx.save();
        ctx.clip();

        // Calculate segment dimensions
        const segmentCenterAngle = startAngle + arc / 2;
        const segmentRadius = (outsideRadius + insideRadius) / 2;
        const segmentWidth = 2 * segmentRadius * Math.sin(arc / 2);
        const segmentHeight = outsideRadius - insideRadius;

        // Position at segment center
        ctx.translate(
          centerX + segmentRadius * Math.cos(segmentCenterAngle),
          centerY + segmentRadius * Math.sin(segmentCenterAngle)
        );
        
        // Rotate to align with segment
        ctx.rotate(segmentCenterAngle + Math.PI / 2);

        // Calculate scale to fit image to segment
        const scaleX = segmentWidth / backgroundImage.width;
        const scaleY = segmentHeight / backgroundImage.height;
        const scale = Math.max(scaleX, scaleY) * 1.2; // Scale up a bit to ensure coverage

        const scaledWidth = backgroundImage.width * scale;
        const scaledHeight = backgroundImage.height * scale;

        // Draw image centered in segment
        ctx.drawImage(
          backgroundImage,
          -scaledWidth / 2,
          -scaledHeight / 2,
        ...
        // Fill segment with semi-transparent background color overlay if it's a gradient
        if (gradientInfo) {
          ctx.globalAlpha = 0.7;

    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (style?.decorativeImages && style.decorativeImages.length > 0) {
        ...
        style.decorativeImages.forEach((decorativeImg, idx) => {
          ...
          const randomRadius =
            insideRadius +
            (outsideRadius - insideRadius) * (0.3 + pseudoRandom1 * 0.4);
          const randomAngle =
            currentStartAngle + currentArc * (0.2 + pseudoRandom2 * 0.6);

          const x = centerX + randomRadius * Math.cos(randomAngle);
          const y = centerY + randomRadius * Math.sin(randomAngle);

          const size = decorativeImg.size || 20;
